...
The first factor appears to largely emphasize protest and police, with additional references to death, innocence, and youth.

The second (GLS2) factor has negative loadings for police images, but high loadings for many of the components we associated with legitimizing content, including images of Michael Brown, references to race, innocence, Michael Brown's name, and active killing. Images of and textual references to protest and police, along with mentions of youth and negative protest load strongly onto the second factor. References to Michael Brown's name and images of Michael brown load negatively onto this factor. This factor is thus mostly oriented around the protests, and with not much attention paid to Michael Brown or his death.
...
Figure \@ref(fig:dim-analysis) displays the models fit to these data. African American oriented sources are roughly less likely to emphasize the first factor than are general public oriented sources (General Public = `r round(fact1_preds$.value[2], 2)`, [`r round(fact1_preds$.lower[2], 2)`, `r round(fact1_preds$.upper[2], 2)`]; African American = `r round(fact1_preds$.value[1], 2)`, [`r round(fact1_preds$.lower[1], 2)`, `r round(fact1_preds$.upper[1], 2)`],  $prob_{gp>aa}$ > .99). 

In contrast, the general public news sources are much less likely to emphasize the second factor than African American oriented sources (General Public = `r round(fact2_preds$.value[2], 2)`, [`r round(fact2_preds$.lower[2], 2)`, `r round(fact2_preds$.upper[2], 2)`]; African American = `r round(fact2_preds$.value[1], 2)`, [`r round(fact2_preds$.lower[1], 2)`, `r round(fact2_preds$.upper[1], 2)`],  $prob_{gp>aa}$ < .01). 

Emphasis on the third factor is roughly equal between these two groups (General Public = `r round(fact3_preds$.value[2], 2)`, [`r round(fact3_preds$.lower[2], 2)`, `r round(fact3_preds$.upper[2], 2)`]; African American = `r round(fact3_preds$.value[1], 2)`, [`r round(fact3_preds$.lower[1], 2)`, `r round(fact3_preds$.upper[1], 2)`],  $prob_{gp>aa}$ = `r round(sum(fact3_mat[,3]>0)/4000, 2)`).
