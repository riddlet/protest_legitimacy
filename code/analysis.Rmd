...
This is the first pass at the analyses registered on the [OSF](https://osf.io/sd58n/). The first pass is some descriptives of the dataset we are working with.
...
mod.dat %>% 
  ...
  ylim(0, .5) +
  theme(legend.position='bottom') -> prot_trust_plot
...
mod.dat %>% 
  ...
  scale_fill_brewer(palette='Greys') +
  ylim(0, .5) +
  theme(legend.position='none') -> pol_trust_plot

cowplot::plot_grid(prot_demog_plot, prot_trust_plot, pol_demog_plot, pol_trust_plot, labels = c('Protest', '', 'Police', ''), rel_widths =c(.75, 1) )
...
The associaton between political leaning and references to these concepts is not as directionally consistent. For both protest ($\beta$ = `r round(protest_trust_slope$b_trust_effect_z, 2)`, [`r round(protest_trust_slope$.lower, 2)`, `r round(protest_trust_slope$.upper, 2)`]) and police references ($\beta$ = `r round(police_trust_slope$b_trust_effect_z, 2)`, [`r round(police_trust_slope$.lower, 2)`, `r round(police_trust_slope$.upper, 2)`]), we are unable to rule out zero as a probable value.
