...
mod.dat %>%
  ...
  summarise(prot_img_prop = mean(Protest, na.rm=T),
            ...
            passive_kill_ref_prop = mean(death_ref),
            pol_ref_prop = mean(police_ref),
            prot_ref_prop = mean(protest_ref),
            neg_protest_ref_prop = mean(neg_protest_ref)) -> cor_dat

write.csv(cor_dat, file = '/Users/travis/Desktop/source_level.csv', row.names = F)
write.csv(mod.dat, file = '/Users/travis/Desktop/document_level.csv', row.names=F)
...
```{r factor-analysis}
eigs <- eigen(M)
fit <- psych::fa(cor_dat[,2:13], nfactors = 3, fm='gls', rotate='varimax')

print(fit$loadings, cut=.3)

```

The first factor appears to be a mixture of references to Michael Brown's death and protest, with references to Michael Brown's name and his innocence, along with references of passive killing, protest, and negative protest references loading highly onto this factor. 

Images of and textual references to protest and police, along with mentions of youth and negative protest load strongly onto the second factor. References to Michael Brown's name and images of Michael brown load negatively onto this factor. This factor is thus mostly oriented around the protests, and with not much attention paid to Michael Brown or his death.

The third factor includes high loadings for race, innocence, youth, active kill, and police.
...
# m_fact4 <- brm(GLS4~trust_effect_z + Demog,
#                  data=mod.dat,
#                  prior = set_prior("normal(0,5)", class = "b"))
# saveRDS(m_fact4,
#         '/Users/travis/Documents/gits/protest_legitimacy/output/fact4.RDS')
# m_fact4 <- readRDS('../output/fact4.RDS')
...
# m_fact4 %>%
#   spread_draws(b_trust_effect_z) %>%
#   median_hdi() -> fact4_trust_slope

# mod.dat %>% 
#   modelr::data_grid(trust_effect_z=c(0,1), Demog) %>%
#   add_fitted_draws(m_fact4, allow_new_levels=T, 
#                    re_formula = NA, transform=T) %>%
#   median_hdi() -> fact4_preds
...
#   xlab('Factor Loading') +
#   #xlim(0, 1) +
#   geom_point(data=mod.dat, aes(x=GLS4, y=Demog), 
#              position=position_jitter(height = .05), size=.5) +
#   theme(axis.text.x = element_text(size=8, angle = 20))  -> fact4_demog_plot
...
#                         guide=guide_legend(title.position='top')) + 
#   geom_point(data=mod.dat, 
#            aes(x=trust_effect_z, y=GLS4)) + 
#   theme(#legend.position='bottom',
#         legend.title=element_text(size=10),
#         legend.text = element_text(size=8)) -> fact4_trust_plot
...
Figure \@ref(fig:dim-analysis) displays the models fit to these data. There are no systematic differences by source audience along the first or third factors. African American oriented sources are roughly equally likely to emphasize the first factor as are general public oriented sources (General Public = `r round(fact1_preds$.value[2], 2)`, [`r round(fact1_preds$.lower[2], 2)`, `r round(fact1_preds$.upper[2], 2)`]; African American = `r round(fact1_preds$.value[1], 2)`, [`r round(fact1_preds$.lower[1], 2)`, `r round(fact1_preds$.upper[1], 2)`],  $prob_{gp>aa}$ < `r round(sum(fact1_mat[,3]>0)/4000, 2)`). Similarly, emphasis on the third factor is roughly equal between these two groups (General Public = `r round(fact3_preds$.value[2], 2)`, [`r round(fact3_preds$.lower[2], 2)`, `r round(fact3_preds$.upper[2], 2)`]; African American = `r round(fact3_preds$.value[1], 2)`, [`r round(fact3_preds$.lower[1], 2)`, `r round(fact3_preds$.upper[1], 2)`],  $prob_{gp>aa}$ = `r round(sum(fact3_mat[,3]>0)/4000, 2)`).

In contrast, the general public news sources are much more likely to emphasize the second factor than African American oriented sources (General Public = `r round(fact2_preds$.value[2], 2)`, [`r round(fact2_preds$.lower[2], 2)`, `r round(fact2_preds$.upper[2], 2)`]; African American = `r round(fact2_preds$.value[1], 2)`, [`r round(fact2_preds$.lower[1], 2)`, `r round(fact2_preds$.upper[1], 2)`],  $prob_{gp>aa}$ > .99). 

The effect of political orientation is generally positive across all three factors, such that more conservative sources are somewhat more likely to emphasize all three factors than more liberal sources, but only for the second factor can we completely rule out zero effect of political orientation  ($factor 1$ = `r round(fact1_trust_slope[1,1], 2)`, [`r round(fact1_trust_slope[1,2], 2)`, `r round(fact1_trust_slope[1,3], 2)`]; $factor 2$ = `r round(fact2_trust_slope[1,1], 2)`, [`r round(fact2_trust_slope[1,2], 2)`, `r round(fact2_trust_slope[1,3], 2)`]; $factor 3$ = `r round(fact3_trust_slope[1,1], 2)`, [`r round(fact3_trust_slope[1,2], 2)`, `r round(fact3_trust_slope[1,3], 2)`]).
