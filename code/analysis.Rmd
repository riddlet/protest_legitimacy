...
### Repeating the analyses without AA-oriented sources 
```{r mlm-audience-PolOrient-genpub-only, fig.cap='Modeled probability of making a reference to Police or Protest as a function of political orientation for general public sources'}

mod.dat %>% #filter out all AA oriented sources
...
police_model_mat <- as.matrix(m_pol_genpub)

protest_model_mat <- as.matrix(m_prot_genpub)
...
mod.dat.genpub %>% 
  modelr::data_grid(trust_effect_z=c(0,1)) %>%
  add_fitted_draws(m_pol_genpub, allow_new_levels=T, re_formula = NA, transform=T) %>%
  median_hdi() -> police_preds

mod.dat.genpub %>% 
  modelr::data_grid(trust_effect_z=c(0,1)) %>%
  add_fitted_draws(m_prot_genpub, allow_new_levels=T, re_formula = NA, transform=T) %>%
  median_hdi() -> protest_preds
...
cowplot::plot_grid(plotcol2, legend, 
                   nrow=1, rel_widths =c(.75, 1, .2) )

```

This text is not real, I just copy pasted from the other similar plot/model so I could (hopefully) extract the correct information from the models so I can understand them. 
