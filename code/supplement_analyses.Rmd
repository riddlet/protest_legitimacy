...
subtitle: "Preregistered Analyses Supplement"
author: "Travis Riddle & Julia Bottesini"
output: 
...
# Change this to TRUE when you wish to rerun all models (e.g., the first time you knit this)
# Models will be saved to ../output/ and will only be reloaded when this is FALSE
# All of these models are tagged with [MODELS] [/MODELS]
# To run only specific models, run them manually
RUN_MODELS <- FALSE
...
In this analysis supplement, we include all the analyses we attempted, and the code we used, whether or not they are included in the final manuscript. When exploratory (non-preregistered) analyses were undertaken, we have labeled them as such. You can find the preregistration on OSF (https://osf.io/sd58n/).

This was our first attemp at interpretating these analyses. We subsequently thought about how these results informed our questions and matched with existing research. Therefore, the explanations and interpretations might differ slightly from the manuscript, including in the wording used to describe concepts.

In terms of wording change, what is referred to as *disruptive protest* in the manuscript was referred to here as *negative protest*. When referring to Michael Brown's death, the word category referred to here as *active kill* is referred to in the manuscript as *active, agent-caused death*, while the category of *passive kill* is referred to as *neutral, non-agentic death*. We keep the original language here to allow readers to more easily connect these analyses with the preregistration document.

# Descriptives
These data are drawn from press coverage of the 2014 shooting of Michael Brown and subsequent protests in Ferguson, MO. We collected all articles mentioning "Ferguson" published in the 10 days following the shooting by the top overall 51 online news organizations and the top 18 African American news organizations, as indicated by Pew Research. Our final data includes the text of all articles mentioning the correct Ferguson (screened manually by research assistants), along with all images included online within each article. In this work, we focus on article headlines, *featured* images, and featured image captions. Featured images are those embedded in the text of the article published online (as opposed to images included as part of a gallery that is linked to within an article). Each featured image was coded by a pair of research assistants as depicting Michael Brown, protest, and/or police.
...
The first part of this supplement follows the pre-registered analyses closely, section by section.

# Police/Protest analyses
## Captions & titles
### Frequency

> A. Establish high frequency of references to police and protest in headlines and featured images

> H: Should not differ much by outlet’s political leaning or Black-oriented, except that perhaps right-wing and mainstream refer to protest a little more or moderately more negatively (e.g., riot, violence)

> 1. Search titles and featured image captions for [police\* | cop\* | officer\* | trooper\* | law enforcement | patrolman | patrolmen | lawman | lawmen] and [protest\* | riot\* | unrest\* | loot\* | demonstrat\* | march* | uprising]. Titles and image captions will be coded for presence or absence of the two categories separately.
...
featured_imgs %>% # select Captions and mutate into shape for merging 
  ...
  rbind(df_arts) -> all_texts # bind to Titles

saveRDS(all_texts, '../output/texts.RDS')
...
scored_dat %>% # calculate overall proportion of prot/pol references
  ...
  distinct() -> plot.dat2 # save for insertion in text
...
> 2. We will estimate multilevel logistic regressions, with title/caption nested within source. We will estimate the outcome as a function of the source audience orientation (M1) and source political leaning (M2).
...
scored_dat %>%
  left_join(df_sources, by = "Source") %>% # add source information
  left_join(df_audience, by = "Source") %>% # add audience information
  ungroup() -> mod.dat
...
m_pol <- brm(police_ref~trust_effect_z + Demog + (1|Source), data=mod.dat,
             ...
             family='bernoulli', save_all_pars=T)

m_prot <- brm(protest_ref~trust_effect_z + Demog + (1|Source), data=mod.dat,
             prior = set_prior("normal(0,5)", class = "b"),
             family='bernoulli', save_all_pars=T)
saveRDS(m_pol, '../output/tempmod_pol.RDS')
saveRDS(m_prot, '../output/tempmod_prot.RDS')
}
#[/MODELS]
...
protest_preds %>%
  ungroup() %>%
  mutate(measure='Protest',
         document='captions+titles') -> plotting_data

police_preds %>%
  ungroup() %>%
  mutate(measure='Police',
         document='captions+titles') %>%
  rbind(plotting_data) -> plotting_data
...
m_prot.nf <- brm(protest_ref~trust_effect_z + Demog + (1|Source), data=nofox.mod,
             prior = set_prior("normal(0,5)", class = "b"),
             family='bernoulli', save_all_pars=T)
saveRDS(m_pol.nf, '../output/tempmod_pol_nf.RDS')
saveRDS(m_prot.nf, '../output/tempmod_prot_nf.RDS')
}
#[/MODELS]
...
##### 2 vs. 3 source audience groups

Refit models after creating a third category of source audience, UK General Audience sources.

```{r 3group-PolOrient}
# Uncomment to run models for the first time
# [MODELS]
if(RUN_MODELS==T){
  
m_pol2 <- brm(police_ref~trust_effect_z + Demog2 + (1|Source), data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars=T)

m_prot2 <- brm(protest_ref~trust_effect_z + Demog2 + (1|Source), data=mod.dat,
             prior = set_prior("normal(0,5)", class = "b"),
             family='bernoulli', save_all_pars=T)
saveRDS(m_pol2, '../output/tempmod_pol2.RDS')
saveRDS(m_prot2, '../output/tempmod_prot2.RDS')

}
# [/MODELS]

m_pol2 <- readRDS('../output/tempmod_pol2.RDS')
m_prot2 <- readRDS('../output/tempmod_prot2.RDS')

#bf_prot <- bayes_factor(m_prot, m_prot2) #1.46
#bf_pol <- bayes_factor(m_pol, m_pol2) #23.16


```


##### Removing African-American oriented sources & Adding text type

Here, we examine whether the results of each model change after removing African-American oriented sources.
...
m_prot_TvC <- brm(protest_ref ~ trust_effect_z + Demog + type + (type|Source),
                  data=mod.dat, prior = set_prior("normal(0,5)", class = "b"),
                  family='bernoulli', save_all_pars=T)

m_pol_TvC <- brm(police_ref ~ trust_effect_z + Demog + type + (type|Source),
                 data=mod.dat, prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars=T)
...
The full model suggests that more conservative sources are far more likely to refer to police than more liberal sources ($\beta$ = `r round(police_trust_slope$b_trust_effect_z, 2)`, [`r round(police_trust_slope$.lower, 2)`, `r round(police_trust_slope$.upper, 2)`]). This result holds when removing African-American oriented sources ($\beta$ = `r round(police_trust_slope_genpub$b_trust_effect_z, 2)`, [`r round(police_trust_slope_genpub$.lower, 2)`, `r round(police_trust_slope_genpub$.upper, 2)`]).
...
> 3. We will additionally perform a ‘spotlight’ analysis – examining the model estimates (from M2) and observed data for the 3 most liberal and 3 most conservative sources to more concretely highlight linguistic patterns.
...
After completing the analysis above we realized it was not as informative as we had hoped. Therefore, we chose not run other spotlight analyses, despite having preregistered them.

## Focusing on negative protest words

> 4. Steps 2-3 will be repeated for negative protest words [riot\*|loot\*],(...)
...
m_prot_neg <- brm(neg_protest_ref~trust_effect_z + Demog + (1|Source),
                  data=mod.dat, prior = set_prior("normal(0,5)", class = "b"),
                  family='bernoulli', save_all_pars = T)
saveRDS(m_prot_neg, '../output/tempmod_prot_neg.RDS')
}
# [/MODELS]
...
neg_protest_preds %>%
  ungroup() %>%
  mutate(measure='Negative Protest',
         document='captions+titles') %>%
  rbind(plotting_data) -> plotting_data
...
saveRDS(m_prot_neg.nf,
        '../output/tempmod_prot_neg_nf.RDS')
}
#[/MODELS]
...
```{r 3-group-mlm-neg-models}

#[MODELS]
if(RUN_MODELS==T){
m_prot_neg2 <- brm(neg_protest_ref~trust_effect_z + Demog2 + (1|Source),
                  data=mod.dat, prior = set_prior("normal(0,5)", class = "b"),
                  family='bernoulli', save_all_pars=T)
saveRDS(m_prot_neg2, '../output/tempmod_prot_neg2.RDS')
}
#[/MODELS]

m_prot_neg2 <- readRDS('../output/tempmod_prot_neg2.RDS')
#bf_prot_neg <- bayes_factor(m_prot_neg, m_prot_neg2) #4.41
...
#[MODELS]
if(RUN_MODELS==T){
m_prot_neg_genpub <- brm(neg_protest_ref ~ trust_effect_z + (1|Source),
                         data=mod.dat.genpub,
                         prior = set_prior("normal(0,5)", class = "b"),
                         family='bernoulli', save_all_pars=T)
...
m_prot_neg_TvC <- brm(neg_protest_ref ~ trust_effect_z + Demog + type + (type|Source), data=mod.dat,
                      prior = set_prior("normal(0,5)", class = "b"),
                      family='bernoulli', save_all_pars=T, iter = 4000)
...
> 4. Steps 2-3 will be repeated for (...) images categorized as protest, and images categorized as police.
...
saveRDS(m_pol_img, '../output/tempmod_pol_img.RDS')
saveRDS(m_prot_img, '../output/tempmod_prot_img.RDS')
}
#[/MODELS]
...
##### 2 vs. 3 source audience groups
```{r 3group-prot-pol-image}
# [MODELS]
if(RUN_MODELS==T){
m_pol_img2 <- brm(Police~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars = T)
saveRDS(m_pol_img2, '../output/tempmod_pol_img2.RDS')

m_prot_img2 <- brm(Protest~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars=T)
saveRDS(m_prot_img2, '../output/tempmod_prot_img2.RDS')
}
#[/MODELS]

m_pol_img2 <- readRDS('../output/tempmod_pol_img2.RDS')
#bf_pol_img <- bayes_factor(m_pol_img, m_pol_img2) #30.46

m_prot_img2 <- readRDS('../output/tempmod_prot_img2.RDS')
#bf_prot_img <- bayes_factor(m_prot_img, m_prot_img2) #23.83


```

##### Removing African-American oriented sources
...
>B. Examine frequency of references in headlines to (a) Black, (b) unarmed, (c) teen, (d) Michael Brown, (e) killed.

>H: left leaning, Black-oriented news media should provide at least moderately more info that appears to legitimize protest by raising the possibility that the police erred because MB was an unarmed, black, teen who was killed.


## Captions, Titles, & Photos

>1. Search titles and featured image captions for [Black | African American | white | Caucasian | race | ethnicity | unarmed | weaponless | innocent | teen\* | youth | young | adolescent | child | graduat\* | high school | Michael Brown | Michael | Mike | Brown | kill\* | gun\* down | mow\* down | murder\* | slaughter\* | butcher\* | execute\* | massacre\*]. Titles and captions will be coded as in step ~~B1~~ A1 (i.e. present/absent)

>2. Add human-coded featured images of Michael Brown (any type) to form combined word/image outcome
...
>3. Estimate multilevel logistic regressions, with title/caption nested within source, using source audience orientation and source political leaning as predictors; (...)
...
saveRDS(m_combined, '../output/combined_img_text.RDS')
}
#[/MODELS]
...
combined_preds %>%
  ungroup() %>%
  mutate(measure='Legitimizing',
         document='captions+titles+photos') %>%
  rbind(plotting_data) -> plotting_data
...
##### 2 vs. 3 source audience groups
```{r 3grooups-combined-analysis}
# [MODELS]
if(RUN_MODELS==T){
m_combined2 <- brm(any_ref~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars = T)

saveRDS(m_combined2, '../output/combined_img_text2.RDS')
}
# [/MODELS]

m_combined2 <- readRDS('../output/combined_img_text2.RDS')
#bf_combined <- bayes_factor(m_combined, m_combined2)
```
...
>4. Repeat step 3 for words alone
...
saveRDS(m_combined.w, '../output/combined_text.RDS')
}
#[/MODELS]
...
```{r 3groups-macro-cat-words-only}
#[MODELS]
if(RUN_MODELS==T){
m_combined.w2 <- brm(target_ref~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars = T)
saveRDS(m_combined.w2, '../output/combined_text2.RDS')
}
#[/MODELS]
m_combined.w2 <- readRDS('../output/combined_text2.RDS')
#bf_combined.w <- bayes_factor(m_combined.w, m_combined.w2) #4.01
```


## Photos only

>5. Repeat step 3 for images alone
...
saveRDS(m_combined.pix, '../output/combined_pix.RDS')
}
#[/MODELS]
...
```{r 3groups-macro-cat-imgs-only}
#[MODELS]
if(RUN_MODELS==T){
m_combined.pix2 <- brm(mb~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars = T)
saveRDS(m_combined.pix2,
        '../output/combined_pix2.RDS')
}
#[/MODELS]

m_combined.pix2 <- readRDS('../output/combined_pix2.RDS')
#bf_combined.pix <- bayes_factor(m_combined.pix, m_combined.pix2) #.01
...
>6. Isolate active kill words [kill\* | gun\* down | mow\* down | murder\* | slaughter\* | butcher\*| execute\* | massacre\*] (...), repeat step 3
...
```{r 3groups-mod-active-kill}
#[MODELS]
if(RUN_MODELS==T){
m_active.kill2 <- brm(target_ref~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars = T)
saveRDS(m_active.kill2,
        '../output/active_kill2.RDS')
}
#[/MODELS]
m_active.kill2 <- readRDS('../output/active_kill2.RDS')
#bf_active.kill <- bayes_factor(m_active.kill, m_active.kill2) #17.49
...
## Passive Kill Words
>6. (...) and search for [dead | death | died | perish\* | shot | shoot\* | fatal\*], repeat step 3.
...
saveRDS(m_passive.kill, '../output/passive_kill.RDS')
}
#[/MODELS]
...
```{r 3groups-mod-passive-kill}
#[MODELS]
if(RUN_MODELS==T){
m_passive.kill2 <- brm(target_ref~trust_effect_z + Demog2 + (1|Source),
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"),
                 family='bernoulli', save_all_pars = T)

saveRDS(m_passive.kill2, '../output/passive_kill2.RDS')
}
#[/MODELS]
m_passive.kill2 <- readRDS('../output/passive_kill2.RDS')
#bf_passive.kill <- bayes_factor(m_passive.kill, m_passive.kill2) #1.07
...
From the preregisteration:
>7. Compute source-level counts and proportions of the following text and image categories
(with image proportions being out of all featured images):
...
• [kill\* | gun\* down | mow\* down | murder\* | slaughter\* | butcher\* | execut\* |
massacr\*]
...
>8. Examine a correlation plot of the above variables

>9. Conduct a factor analysis for the above variables, and retain any factor(s) with
eigenvalues > 1 and repeat step 3 for these factors.

>10. Repeat steps 1-6 where the outcome is scored as the count of the number of hits per
article (e.g. compute the total number of times a source has a featured MB image + hits for the words in step). We model these data using multilevel regressions for count data, using poisson regressions if the counts are not overdispersed, and negative binomial if they are.
...
all_texts %>%
  mutate(text = gsub('law enforcement', 
                     'law_enforcement', text, ignore.case=T)) %>%
  mutate(text = gsub('african american',
                     'african_american', text, ignore.case=T)) %>%
  mutate(text = gsub('high school',
                     'high_school', text, ignore.case=T)) %>%
  ...
  mutate(text = gsub('mike brown',
                     'mike_brown', text, ignore.case=T)) %>%
  mutate(text = gsub('mow.* down',
                     'mow_down', text, ignore.case=T)) -> df # save to df
...
kill_words <- data.frame(word = c('kill.*', 'gun.*_down', 'mow_down', 
                                    'murder.*', 'slaughter.*', 'butcher.*', 
                                    'execute.*', 'massacre.*'),
                           kill_ref = rep(1, 8))
...
tidy_articles %>%
  ...
  summarise(race_ref = max(race_ref),
            ...
            death_ref = max(death_ref),
            police_ref = max(police_ref),
            protest_ref = max(protest_ref),
            neg_protest_ref = max(neg_protest_ref)) -> scored_dat

#saveRDS(scored_dat, '../output/scored_texts.RDS')
scored_dat <- readRDS('../output/scored_texts.RDS')
...
#[MODELS]
if(RUN_MODELS==T){
m_race <- brm(race_ref~trust_effect_z + Demog + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
m_race2 <- brm(race_ref~trust_effect_z + Demog2 + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
saveRDS(m_race,'../output/race.RDS')
saveRDS(m_race2,'../output/race2.RDS')
}
#[/MODELS]

m_race <- readRDS('../output/race.RDS')
m_race2 <- readRDS('../output/race2.RDS')
#bf_race <- bayes_factor(m_race, m_race2)


mod.dat %>% 
  ungroup() %>%
  modelr::data_grid(trust_effect_z=c(0,1), Demog) %>%
  add_fitted_draws(m_race, allow_new_levels=T, 
                   re_formula = NA, transform=T) %>%
  median_hdi() -> race_preds
...
#[MODELS]
if(RUN_MODELS==T){
m_inn <- brm(inn_ref~trust_effect_z + Demog + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
m_inn2 <- brm(inn_ref~trust_effect_z + Demog2 + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
saveRDS(m_inn,'../output/innocence.RDS')
saveRDS(m_inn2,'../output/innocence2.RDS')
}
#[/MODELS]
m_inn <- readRDS('../output/innocence.RDS')
m_inn2 <- readRDS('../output/innocence2.RDS')
#bf_inn <- bayes_factor(m_inn, m_inn2)

mod.dat %>% 
  ungroup() %>%
  modelr::data_grid(trust_effect_z=c(0,1), Demog) %>%
  add_fitted_draws(m_inn, allow_new_levels=T, 
                   re_formula = NA, transform=T) %>%
  median_hdi() -> inn_preds

inn_preds %>%
  ungroup() %>%
  mutate(measure='innocence',
         document='captions+titles') %>%
  rbind(plotting_data) -> plotting_data

#[MODELS]
if(RUN_MODELS==T){
m_young <- brm(young_ref~trust_effect_z + Demog + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
m_young2 <- brm(young_ref~trust_effect_z + Demog2 + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
saveRDS(m_young, '../output/young.RDS')
saveRDS(m_young2, '../output/young2.RDS')
}
#[/MODELS]

m_young <- readRDS('../output/young.RDS')
m_young2 <- readRDS('../output/young2.RDS')
#bf_young <- bayes_factor(m_young, m_young2)

mod.dat %>% 
  ungroup() %>%
  modelr::data_grid(trust_effect_z=c(0,1), Demog) %>%
  add_fitted_draws(m_young, allow_new_levels=T, 
                   re_formula = NA, transform=T) %>%
  median_hdi() -> young_preds

young_preds %>%
  ungroup() %>%
  mutate(measure='young',
         document='captions+titles') %>%
  rbind(plotting_data) -> plotting_data
#[MODELS]
if(RUN_MODELS==T){
m_mb <- brm(mb_ref~trust_effect_z + Demog + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)
m_mb2 <- brm(mb_ref~trust_effect_z + Demog2 + (1|Source),
              data=mod.dat,
              prior = set_prior("normal(0,5)", class = "b"),
              family='bernoulli', save_all_pars = T)

saveRDS(m_mb, '../output/mb.RDS')
saveRDS(m_mb2, '../output/mb2.RDS')
}
#[/MODELS]
m_mb <- readRDS('../output/mb.RDS')
m_mb2 <- readRDS('../output/mb2.RDS')
#bf_mb <- bayes_factor(m_mb, m_mb2)

mod.dat %>% 
  ungroup() %>%
  modelr::data_grid(trust_effect_z=c(0,1), Demog) %>%
  add_fitted_draws(m_mb, allow_new_levels=T, 
                   re_formula = NA, transform=T) %>%
  median_hdi() -> mb_preds
...
featured_imgs %>%
  ...
  mutate(ID = PictureID) %>% 
  select(Source, ID, Protest, Police, MB) %>%
  mutate(type='Pic, No Caption') %>%
  full_join(scored_dat) -> mod.dat

mod.dat %>%
  group_by(Source) %>%
  summarise(prot_img_prop = mean(Protest, na.rm=T),
            pol_img_prop = mean(Police, na.rm=T),
            mb_img_prop = mean(MB, na.rm=T),
            race_ref_prop = mean(race_ref, na.rm=T),
            inn_ref_prop = mean(inn_ref, na.rm=T),
            young_ref_prop = mean(young_ref, na.rm=T),
            mb_ref_prop = mean(mb_ref, na.rm=T),
            active_kill_ref_prop = mean(kill_ref, na.rm=T),
            passive_kill_ref_prop = mean(death_ref, na.rm=T),
            pol_ref_prop = mean(police_ref, na.rm=T),
            prot_ref_prop = mean(protest_ref, na.rm=T),
            neg_protest_ref_prop = mean(neg_protest_ref, na.rm=T)) -> cor_dat

write.csv(cor_dat, file = '../output/source_level_cors.csv', row.names = F)
write.csv(mod.dat, file = '../output/document_level_cors.csv', row.names=F)
...
```{r factor-analysis}
eigs <- eigen(M)
fit <- psych::fa(cor_dat[,2:13], nfactors = 3, fm = 'minres', rotate='oblimin')

print(fit$loadings, cut=.3)

```

Factor 1 appears to represent online news sources that frame Ferguson as a disruptive protest at a police murder of an unarmed, Black, youth (who is not named).  


Factor 2 appears to represent online news sources that (very visually) frame Ferguson as a conflict between police and protestors over a death.  These sources appear to deracialize the killing as they are very unlikely to show Michael Brown or mention race. That these sources are also less likely to use injunctive death verbs further suggests that this framing does not emphasize of Michael Brown’s killing as unjust, thus potentially casting the Ferguson protests as illegitimate.

The third factor includes high loadings for police and protest images, negative protest references, Michael Brown, passive killing, and innocence.
...
#[MODELS]
if(RUN_MODELS==T){
m_fact1 <- brm(MR1~trust_effect_z + Demog,
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"))
saveRDS(m_fact1, '../output/fact1.RDS')

m_fact2 <- brm(MR2~trust_effect_z + Demog,
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"))
saveRDS(m_fact2, '../output/fact2.RDS')

m_fact3 <- brm(MR3~trust_effect_z + Demog,
                 data=mod.dat,
                 prior = set_prior("normal(0,5)", class = "b"))
saveRDS(m_fact3, '../output/fact3.RDS')
}
#[/MODELS]
...
mod.dat %>% 
  ...
  xlab('Factor Loading') +
  #xlim(0, 1) +
  geom_point(data=mod.dat, aes(x=MR1, y=Demog), 
...
mod.dat %>% 
  ...
  xlab('Factor Loading') +
  #xlim(0, 1) +
  geom_point(data=mod.dat, aes(x=MR2, y=Demog), 
             position=position_jitter(height = .05), size=.5) +
...
mod.dat %>% 
  ...
  xlab('Factor Loading') +
  #xlim(0, 1) +
  geom_point(data=mod.dat, aes(x=MR3, y=Demog), 
             position=position_jitter(height = .05), size=.5) +
...
fact1_preds %>%
  ...
  mutate(measure='factor1',
         document='factor_analysis') %>%
  rbind(plotting_data) -> plotting_data

fact2_preds %>%
  ungroup() %>%
  mutate(measure='factor2',
         document='factor_analysis') %>%
  rbind(plotting_data) -> plotting_data

fact3_preds %>%
  ungroup() %>%
  mutate(measure='factor3',
         document='factor_analysis') %>%
  rbind(plotting_data) -> plotting_data


write.csv(plotting_data, 
          '../output/plotting_data_demogs.csv', row.names = FALSE)
...
Figure \@ref(fig:dim-analysis) displays the models fit to these data. In comparison to the general public, African American oriented sources are less likely to emphasize the first factor (General Public = `r round(fact1_preds$.value[2], 2)`, [`r round(fact1_preds$.lower[2], 2)`, `r round(fact1_preds$.upper[2], 2)`]; African American = `r round(fact1_preds$.value[1], 2)`, [`r round(fact1_preds$.lower[1], 2)`, `r round(fact1_preds$.upper[1], 2)`],  $prob_{gp>aa}$ = `r round(sum(fact1_mat[,3]>0)/4000, 2)`) and second factor (General Public = `r round(fact2_preds$.value[2], 2)`, [`r round(fact2_preds$.lower[2], 2)`, `r round(fact2_preds$.upper[2], 2)`]; African American = `r round(fact2_preds$.value[1], 2)`, [`r round(fact2_preds$.lower[1], 2)`, `r round(fact2_preds$.upper[1], 2)`],  $prob_{gp>aa}$ > .99). 

In contrast, the general public news sources are much less likely to emphasize the third factor than African American oriented sources (General Public = `r round(fact3_preds$.value[2], 2)`, [`r round(fact3_preds$.lower[2], 2)`, `r round(fact3_preds$.upper[2], 2)`]; African American = `r round(fact3_preds$.value[1], 2)`, [`r round(fact3_preds$.lower[1], 2)`, `r round(fact3_preds$.upper[1], 2)`],  $prob_{gp>aa}$ = `r round(sum(fact3_mat[,3]>0)/4000, 2)`).

The effect of political orientation is generally positive across all three factors, such that more conservative sources are somewhat more likely to emphasize all three factors than more liberal sources, but we cannot completely rule out zero effect of political orientation for any factor ($factor 1$ = `r round(fact1_trust_slope[1,1], 2)`, [`r round(fact1_trust_slope[1,2], 2)`, `r round(fact1_trust_slope[1,3], 2)`]; $factor 2$ = `r round(fact2_trust_slope[1,1], 2)`, [`r round(fact2_trust_slope[1,2], 2)`, `r round(fact2_trust_slope[1,3], 2)`]; $factor 3$ = `r round(fact3_trust_slope[1,1], 2)`, [`r round(fact3_trust_slope[1,2], 2)`, `r round(fact3_trust_slope[1,3], 2)`]).

## Model Comparison for segregating out UK sources
Because there are likely some differences in the way that North American and European news sources cover this event, we wanted to be sure that we were not overlooking systematic differences between these two subgroups of news organizations. However, we were also cognizant that simply looking for differences between these groups has the potential to mislead. Since there are only 6 UK sources, estimates for that group are likely to be highly variable. So, we used a bayes factor model comparison procedure to evaluate whether the data more strongly support statistical models with just two source audience groupings (African American vs General Public, & UK). Bayes factor model comparisons yield a factor (K) that indicates the strength of the support for model 1 over model 2. Kass & Raftery provide some guidance in interpreting K:
...
We ran model comparisons for models of police and protest words, disruptive protest words, police and protest images, legitimizing words and images, legitimizing words, michael brown images, injunctive and descriptive death verbs, and words for race, innocence, young, and references to Michael Brown's name

```{r bf}
# tempdf <- data.frame(model = c('Protest Images', 'Police Images', 
#                                'Michael Brown Images', 'Race Words', 
#                                'Unarmed Words', 'Youth Words', 
#                                'Michael Brown Words', 'Injunctive Death Verbs',
#                                'Descriptive Death Verbs', 'Police Words', 
#                                'Protest Words', 'Disruptive Protest Words',
#                                'Legitimizing Words and Images', 
#                                'Legitimizing Words'),
#                      bf = c(bf_prot_img$bf, bf_pol_img$bf,
#                             bf_combined.pix$bf, bf_race$bf,
#                             bf_inn$bf, bf_young$bf, bf_mb$bf,
#                             bf_active.kill$bf, bf_passive.kill$bf, 
#                             bf_pol$bf, bf_prot$bf, bf_prot_neg$bf,
#                             bf_combined$bf, bf_combined.w$bf))
# 
# tempdf
```

Of these 14 model comparisons, just two indicate that the data is in favor of the models with three sources: models for Michael Brown words and Michael Brown Pictures. 
...
saveRDS(m_nb_any, '../output/negbin_any.RDS')
}
#[/MODELS]
...
saveRDS(m_nb_any_pix, '../output/negbin_any_andpix.RDS')
}
#[/MODELS]
...
saveRDS(m_nb_active.kill, '../output/active_kill_nb.RDS')
}
#[/MODELS]
...
saveRDS(m_nb_passive.kill, '../output/passive_kill_nb.RDS')
}
#[/MODELS]
...
>Due to important temporal dynamics that are fundamental to this particular event, we will
also explore coverage over time. These analyses serve primarily to ensure that any observed patterns are not due to outliers related to the change in coverage on a particular day (e.g. flare- ups in protest and police response on August 14th and 18th).

>1. Create plots of audience means by day for:
• Words calculated in Step A1
• Police/protest images (as in step B7)
• Combined word/image count calculated in Step B10

>2. Repeat plots for political leaning category (3 conservative, 3 liberal) means
